...
public class VotanteRepository {
	...
	public void insert(Votantes votante) {
		...
		try {
			preparedStatement = conn
					//TODO
					.prepareStatement("INSERT INTO votante (id,nombre,voto, email, code) VALUES (?, ?, ?, ?, ?)");
			preparedStatement.setInt(1, id);
			preparedStatement.setString(2, votante.getNombre());
			preparedStatement.setInt(3, votante.getVoto());
			preparedStatement.setString(4, votante.getEmail());
			preparedStatement.setString(5, votante.getCode());
	...
	public List<Votantes> listAll() {
		...
		try {
			...
			ResultSet resultSet = preparedStatement.executeQuery();
			while (resultSet.next()) {
				Votantes votante = new Votantes(0, null, 0, null, null);
				votante.setId(resultSet.getInt("id"));
				votante.setNombre(resultSet.getString("nombre"));
				votante.setVoto(resultSet.getInt("voto"));
				votante.setEmail(resultSet.getString("email"));
				votante.setCode(resultSet.getString("code"));
				lista.add(votante);
